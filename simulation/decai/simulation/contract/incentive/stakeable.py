...
class Stakeable(IncentiveMechanism):
    ...
    def get_next_add_data_cost(self, data, classification) -> float:
        ...
        print('Print Classification and Entropy')
        print(classification)
        print(entropy(classification))
        result = self.cost_weight * 60 / int(math.sqrt(time_since_last_update_s)) / entropy(classification)
        result = int(result)
    ...
    # def handle_add_data(self, contributor_address: Address, submitter: str, msg_value: float, data, classification) -> (float, bool): 
        # cost = int(self.get_next_add_data_cost(data, classification) / self.num_good_data_per_user[submitter])
